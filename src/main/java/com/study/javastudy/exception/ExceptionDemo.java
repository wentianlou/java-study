 ...
 *
 * 分类：
 *     非检查异常：(编译器不要求强制处置的异常)，javac在编译时，不会提示和发现这样的异常，不要求在程序处理这些异常。对于这些异常，我们应该修正代码，而不是去通过异常处理器处理 。
 *        这样的异常发生的原因多半是代码写的有问题，如除0错误ArithmeticException。
 *     检查异常：(编译器要求必须处置的异常)，javac强制要求程序员为这样的异常做预备处理工作（使用try…catch…finally或者throws）。在方法中要么用try-catch语句捕获它并处理，
